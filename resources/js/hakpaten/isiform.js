document.addEventListener('DOMContentLoaded', () => {
  ...
  if (jumlahEl && container && tpl) {
    ...
    // update ketika jumlah berubah (preserve value yang sudah diisi)
    const rerenderPreserve = () => {
      oldInventors = getCurrentInventorValues();
      renderInventors();
    };

    jumlahEl.addEventListener('change', rerenderPreserve);

    // optional: realtime pas ngetik angka
    let t;
    jumlahEl.addEventListener('input', () => {
      clearTimeout(t);
      t = setTimeout(rerenderPreserve, 200);
    });
  }
