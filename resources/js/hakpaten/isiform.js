...
if (jumlahEl && container && tpl && btnMinus && btnPlus) {
  let oldInventors = [];
  const oldJsonEl = document.getElementById('old-inventor-data');
  if (oldJsonEl) {
    try {
  ...
  const getCurrent = () =>
    Array.from(container.querySelectorAll('input.inventor-input')).map(i => i.value);
  ...
  const setJumlah = (n) => {
    n = clamp(n);
    oldInventors = getCurrent();   // preserve sebelum re-render
    jumlahEl.value = n;
    render(n);
  };

  // initial render
  const init = clamp(parseInt(jumlahEl.value || '1', 10) || 1);
  jumlahEl.value = init;
  render(init);

  btnMinus.addEventListener('click', () => setJumlah((parseInt(jumlahEl.value, 10) || 1) - 1));
  btnPlus.addEventListener('click', () => setJumlah((parseInt(jumlahEl.value, 10) || 1) + 1));
}
  }
