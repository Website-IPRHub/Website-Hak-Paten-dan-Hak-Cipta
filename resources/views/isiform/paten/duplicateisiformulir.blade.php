...
{{-- Taruh include di luar blok PHP agar menu tetap muncul --}}
...
<div class="paten-form-page">
  ...
  @error('konsultanpaten') <small style="color:red">{{ $message }}</small> @enderror
    ...
    <div class="actions-left">
        {{-- Tombol ini yang akan memicu SweetAlert --}}
        <button 
            type="button" 
            id="nextLinkIsiform" 
            class="btn-next"
            data-save-url="{{ route('dup.isiform.store') }}" 
    ...
    try {
      ...
      <div class="actions-download">
        ...
        <select id="doc_type" class="input" style="width:220px;">
            ...
            <option value="{{ route('dup.isiform.store') }}">Form Paten</option>
            <option value="{{ route('dup.invensi.store') }}">Kepemilikan Invensi</option>
            <option value="{{ route('dup.pengalihanhak.store') }}">Surat Pengalihan Hak</option>
        </select>
