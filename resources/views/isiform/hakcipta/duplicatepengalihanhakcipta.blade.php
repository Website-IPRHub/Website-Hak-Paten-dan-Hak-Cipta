...
<div class="paten-form-page">
  <div class="judul">
    ...
    <form class="form" method="POST" action="{{ route('dup.pengalihanhakCipta.store') }}">
    @csrf
    ...
    <div class="actions-bar">
        ...
        <button
            type="button"
            class="btn-prev"
            data-fallback="{{ route('dup.hakcipta.isiform.suratpernyataan') }}"
            onclick="(history.length > 1) ? history.back() : (window.location.href=this.dataset.fallback)"
        >
            &laquo; Sebelumnya
        </button>

        <a class="btn-next" href="{{ route('dup.hakcipta.isiform.peralihanverifcipta') }}">
            Selanjutnya &raquo;
        </a>
        </div>
