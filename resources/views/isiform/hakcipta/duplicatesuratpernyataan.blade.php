...
<div class="paten-form-page">
  ...
  <form class="form" method="POST" action="{{ route('dup.pernyataanCipta.store') }}">
    @csrf
    ...
    <div class="actions-bar">
      <div class="actions-left">
        <button
          type="button"
          class="btn-prev"
          data-fallback="{{ route('dup.hakcipta.isiform.formpendaftaran') }}"
          onclick="(history.length > 1) ? history.back() : (window.location.href=this.dataset.fallback)"
        >
          &laquo; Sebelumnya
        </button>

        <a class="btn-next" href="{{ route('dup.hakcipta.isiform.pengalihanhak') }}">
          Selanjutnya &raquo;
        </a>
      </div>
