    ...
    <div class="content-isi">
      ...
      <div class="hero-buttons-start">
        <div class="button-upload">
          <form
            id="draftForm"
            action="{{ route('patenverif.upload.invensi', ['verif' => $verif->id]) }}"
            method="POST"
  ...
  @endif
    ...
    <div class="actions-bar">
      <button type="button" class="btn-prev"
        data-fallback="{{ route('patenverif.formpermohonan', ['verif' => $verif->id]) }}"
        onclick="(history.length > 1) ? history.back() : (window.location.href=this.dataset.fallback)">
        &laquo; Sebelumnya
      </button>

      <a class="btn-next" href="{{ route('patenverif.pengalihanhak', ['verif' => $verif->id]) }}">
        Selanjutnya &raquo;
      </a>
    </div>
