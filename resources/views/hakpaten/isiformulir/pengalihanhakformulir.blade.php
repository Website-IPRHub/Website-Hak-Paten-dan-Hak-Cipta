...
<div class="inv-wrap">
  <div class="judul">
    ...
    <p>Isi formulir ini untuk mendapatkan dokumen pengalihan hak</p>
  </div>

  <form class="form inv-form-full" method="POST" action="{{ route('pengalihanhak.store') }}">
    @csrf
    ...
    <div class="col">
      <div class="field">
        ...
        <input type="text" class="input" name="judul_paten"
              placeholder="Masukkan judul paten"
              value="{{ old('judul_paten') }}" required>
        @error('judul_paten') <small style="color:red">{{ $message }}</small> @enderror
      </div>
    </div>
