  ...
  <div class="section-inner">
    <div class="content-isi">
      ...
      <div class="hero-buttons-start">
        <div class="button-upload">
          <form
            id="draftForm"
            action="{{ route('ciptaverif.upload.pengalihan', ['verif' => $verif->id]) }}"
            method="POST"
    ...
    <div class="actions-bar">
      <button type="button" class="btn-prev"
        data-fallback="{{ route('ciptaverif.suratpernyataan', ['verif' => $verif->id]) }}"
        onclick="(history.length > 1) ? history.back() : (window.location.href=this.dataset.fallback)">
        &laquo; Sebelumnya
      </button>

      <a class="btn-next" href="{{ route('ciptaverif.scanktp',['verif' => $verif->id])}}">
        Selanjutnya &raquo;
      </a>
    </div>
